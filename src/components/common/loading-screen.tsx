...
import {ChartIcon, BrainIcon, LoadingIcon, ShieldIcon} from '@/components/ui/icons';
              ...
              return (
                ...
                >
                  {isCompleted ? (
                    ...
                    <LoadingIcon className="w-4 h-4 text-psychology-primary animate-spin" />
                  ) : (
...
export function SimpleLoader({ message = "加载中..." }: { message?: string }) {
  return (
    <div className="flex items-center justify-center gap-3 p-4">
      <LoadingIcon className="w-5 h-5 animate-spin text-psychology-primary" />
...
export function InlineLoader({ size = "sm" }: { size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <LoadingIcon className={`${sizeClasses[size]} animate-spin text-psychology-primary`} />
  );
}